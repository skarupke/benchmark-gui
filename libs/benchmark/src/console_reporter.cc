...
void ConsoleReporter::PrintRunData(const Run& result) {
  ...
  if (result.items_per_second > 0) {
      double nanoseconds_per_item = 1000000000.0 / result.items_per_second;
    items =
        StrCat(" ", HumanReadableNumber(result.items_per_second), " items/s ");
    while (items.size() < 18)
        items.push_back(' ');
    items += StringPrintF(" %.2f ns/item", nanoseconds_per_item);
    while (items.size() < 33)
        items.push_back(' ');
  }
  ...
  if (!items.empty()) {
    printer(Out, COLOR_DEFAULT, " %s", items.c_str());
  }
