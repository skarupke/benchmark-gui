...
struct BaseArrayView
{
    ...
    Derived subview(size_t index) const
    {
        if (size() <= index)
            return Derived(end_, end_);
        else
            return Derived(begin_ + index, end_);
    }

    Derived subview(size_t index, size_t length) const
    {
    ...
    {
        return begin_ != end_ && *begin_ == value;
    }
    ...
    {
        return begin_ != end_ && end_[-1] == value;
    }
...
struct ArrayView : detail::BaseArrayView<T, ArrayView<T>>
{
    ...
    ArrayView() = default;
    ArrayView(std::initializer_list<T> il)
        : Base(il.begin(), il.end())
    {
    }
    ArrayView(std::vector<typename std::remove_const<T>::type> & vec)
        : Base(vec.data(), vec.data() + vec.size())
    {
    }
    ArrayView(const std::vector<typename std::remove_const<T>::type> & vec)
        : Base(vec.data(), vec.data() + vec.size())
    {
    }
    template<size_t S>
    ArrayView(std::array<typename std::remove_const<T>::type, S> & array)
        : Base(array.data(), array.data() + array.size())
    {
    }
    template<size_t S>
    ArrayView(const std::array<typename std::remove_const<T>::type, S> & array)
        : Base(array.data(), array.data() + array.size())
    {
    }
};
...
struct StringView : detail::BaseArrayView<C, StringView<C>>
{
    ...
    StringView(C (&static_string)[Size]) noexcept
        : Base(static_string, static_string + (static_string[Size - 1] ? Size : Size - 1))
    {
    }
    ...
    StringView<C> substr(size_t index) const
    {
        return this->subview(index);
    }
    StringView<C> substr(size_t index, size_t length) const
    {
