...
uint64_t smallest_descendant(It begin, uint64_t length, uint64_t first_child, uint64_t first_grandchild, Compare && compare)
{
    ...
    if (first_grandchild >= length)
        return first_child + static_cast<int>(second_child != length && compare(begin[second_child], begin[first_child]));
...
uint64_t largest_descendant(It begin, uint64_t length, uint64_t first_child, uint64_t first_grandchild, Compare && compare)
{
    uint64_t second_child = first_child + 1;
    if (first_grandchild >= length)
        return first_child + static_cast<int>(second_child != length && compare(begin[first_child], begin[second_child]));
    uint64_t second_grandchild = first_grandchild + 1;
    if (second_grandchild == length)
...
void push_down_min(It begin, typename std::iterator_traits<It>::value_type value, uint64_t index, uint64_t length, Compare&& compare)
{
    ...
    for (;;)
    {
        ...
        if (last_grandchild < length)
        {
            ...
            uint64_t smallest = compare(begin[min_first_half], begin[min_second_half]) ? min_first_half : min_second_half;
...
{
    using std::swap;
    uint64_t smaller_child = first_child_index(index);
    uint64_t second_child = smaller_child + 1;
    if (compare(begin[second_child], begin[smaller_child]))
...
{
    using std::swap;
    uint64_t bigger_child = first_child_index(index);
    uint64_t second_child = bigger_child + 1;
    if (compare(begin[bigger_child], begin[second_child]))
...
template<typename It, typename Compare>
void custom_make_heap(It begin, It end, Compare && compare)
{
    using std::swap;
    uint64_t length = end - begin;
    uint64_t index = length / 2;
    ...
    if ((length & 1) == 0)
    {
        --index;
        uint64_t first_child = minmax_heap_helpers::first_child_index(index);
        if (compare(begin[index], begin[first_child]))
            swap(begin[index], begin[first_child]);
        if (index == 0)
            return;
    }
    // optimization: half of all the items will have no grandchildren. this
    ...
    if (length != 4)
    {
        ...
        for (--index;; --index)
        {
            uint64_t larger_child = minmax_heap_helpers::first_child_index(index);
            uint64_t second_child = larger_child + 1;
            if (compare(begin[larger_child], begin[second_child]))
                larger_child = second_child;
            if (compare(begin[index], begin[larger_child]))
                swap(begin[index], begin[larger_child]);
            if (index == lowest_index_with_no_grandchildren)
                break;
    ...
    for (; index != 0;)
    {
        ...
        for (;;)
        {
            uint64_t larger_child = minmax_heap_helpers::first_child_index(move_down_index);
            if (larger_child >= length)
                break;
            uint64_t second_child = larger_child + 1;
            if (second_child != length && compare(begin[larger_child], begin[second_child]))
                larger_child = second_child;
            if (!compare(value, begin[larger_child]))
                break;
            begin[move_down_index] = std::move(begin[larger_child]);
            move_down_index = larger_child;
        }
        begin[move_down_index] = std::move(value);
    }
}
template<typename It>
void custom_make_heap(It begin, It end)
{
    return custom_make_heap(begin, end, std::less<>{});
}
...
inline It largest_child(It first_child_it, Compare && compare)
{
    if constexpr (D == 1)
        ...
        return first_child_it + static_cast<int>(compare(first_child_it[0], first_child_it[1]));
    else
    {
...
It largest_child(It first_child_it, int num_children, Compare && compare)
{
    ...
    else if constexpr (D == 4)
    {
        ...
        case 2: return first_child_it + static_cast<int>(compare(first_child_it[0], first_child_it[1]));
        default:
            It largest = first_child_it + static_cast<int>(compare(first_child_it[0], first_child_it[1]));
    ...
    else
    {
        ...
        case 4:
        {
            It largest_first_half = first_child_it + static_cast<int>(compare(first_child_it[0], first_child_it[1]));
            It largest_second_half = first_child_it + 2 + static_cast<int>(compare(first_child_it[2], first_child_it[3]));
