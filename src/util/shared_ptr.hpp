...
{
    template<typename>
    friend struct small_shared_ptr;
	...
	~small_shared_ptr() noexcept
	{
        if (storage && storage->ref_count.decrement())
            delete storage;
	}
    ...
    operator small_shared_ptr<const T>() const
    {
        small_shared_ptr<const T> result;
        result.storage = reinterpret_cast<typename small_shared_ptr<const T>::Storage *>(storage);
        storage->ref_count.increment();
        return result;
    }
