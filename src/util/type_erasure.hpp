...
{
    static custom_typeinfo target_type()
    {
        return custom_typeid<T>();
    }
    static void * target(custom_typeinfo info, Storage & storage)
    {
        if (custom_typeid<T>() == info) return &storage.template get_reference<T>();
        else return nullptr;
    }
};
...
struct BaseVTable
{
    ...
    const custom_typeinfo target_type;
    void * (*target)(custom_typeinfo type, Storage & object);
...
struct BaseTypeErasure
{
    ...
    T * target()
    {
        if (vtable) return static_cast<T *>(vtable->target(custom_typeid<T>(), storage));
        else return nullptr;
    }
    template<typename T>
    ...
    custom_typeinfo target_type() const
    {
        if (vtable) return vtable->target_type;
        else return custom_typeid<void>();
    }
...
struct EqualityTypeErasure : BaseTypeErasure<Size, Vtable, Alignment, Allocator>
{
    ...
    friend bool operator==(const EqualityTypeErasure & lhs, const EqualityTypeErasure & rhs)
    {
        if (!lhs.vtable) return !rhs.vtable;
        else return lhs.vtable == rhs.vtable && lhs.vtable->equals(lhs.storage, rhs.storage);
    }
    friend bool operator!=(const EqualityTypeErasure & lhs, const EqualityTypeErasure & rhs)
    {
        return !(lhs == rhs);
...
struct RegularTypeErasure : EqualityTypeErasure<Size, Vtable, Alignment, Allocator>
{
    ...
    friend bool operator<(const RegularTypeErasure & lhs, const RegularTypeErasure & rhs)
    {
        return lhs.vtable < rhs.vtable
                || (!(rhs.vtable < lhs.vtable) && lhs.vtable->less(lhs.storage, rhs.storage));
    }
    friend bool operator>(const RegularTypeErasure & lhs, const RegularTypeErasure & rhs)
    {
        return rhs < lhs;
