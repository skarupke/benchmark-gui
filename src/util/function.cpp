...
/*TEST(function, empty)
{
	...
	try
	{
		::assert::ScopedSetBreakOnThrow disable_break(false);
		empty();
	}
	catch(func::bad_function_call &)
	{
		thrown = true;
	}
    ASSERT_TRUE(thrown);
...
struct TakesAFunction
{
    func::function<void ()> a;
    TakesAFunction(func::function<void ()> a)
        : a(std::move(a))
    {
    }
};
