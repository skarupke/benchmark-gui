...
/*size_t interned_string::garbage_collect()
{
    size_t count = 0;
    std::lock_guard<std::mutex> lock(all_interned_strings_mutex());
    auto & all_strings = all_interned_strings();
    for (auto it = all_strings.begin(); it != all_strings.end();)
    {
...
{
    //interned_string::garbage_collect();
    {
    ...
    //ASSERT_EQ(2, interned_string::garbage_collect());
}
