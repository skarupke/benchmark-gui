...
#include "../boost_test_hashtable_redirect.hpp"
...
{
template<>
struct hash<insert_tests::initialize_from_two_ints>
{
    size_t operator()(const insert_tests::initialize_from_two_ints & val) const;
};
template<>
struct hash<insert_tests::overloaded_constructor>
{
    size_t operator()(const insert_tests::overloaded_constructor & val) const;
};
...
static test::seed_t initialize_seed(243432);

template<typename L, typename R>
bool element_equal(const L & l, const R & r)
{
    return l == r;
}
template<typename L1, typename L2, typename R1, typename R2>
bool element_equal(const std::pair<L1, L2> & l, const std::pair<R1, R2> & r)
{
...
void unique_insert_tests1(X*, test::random_generator generator)
{
            ...
            it != v.end(); ++it)
    {
        ...
        BOOST_TEST(element_equal(*r1.first, *r2.first));
...
void unique_emplace_tests1(X*, test::random_generator generator)
{
            ...
            it != v.end(); ++it)
    {
        ...
        BOOST_TEST(r1.second == r2.second);
        element_equal(*r1.first, *r2.first);
...
void map_insert_range_test2(X*, test::random_generator generator)
{
    ...
    > list;
    test::random_values<
        boost::unordered_map<BOOST_DEDUCED_TYPENAME X::key_type, test::implicitly_convertible, test::hash, test::equal_to>
    > v(1000, generator);
    list l(v.begin(), v.end());
...
boost_test_hash_set<test::movable,
...
UNORDERED_TEST(insert_tests2,
    (/*(test_multimap_std_alloc)(test_multiset)*/(test_set)(test_map)/*(test_multimap)*/)
    ((default_generator)(generate_collisions))
)
...
UNORDERED_TEST(move_emplace_tests,
    (/*(test_multimap_std_alloc)*/(test_set_std_alloc)(test_set)(test_map)
...
{
    boost_test_hash_set<int> set;
    set.insert({1,2,3,1});
    BOOST_TEST_EQ(set.size(), 3u);
    BOOST_TEST(set.find(1) != set.end());
    BOOST_TEST(set.find(4) == set.end());

    boost_test_hash_set<initialize_from_two_ints> set2;
...
{
    boost_test_hash_map1<std::string, std::string> map;
	map.insert({});
...
{
    boost_test_hash_map1<int, overloaded_constructor> x;
...
{
    boost_test_hash_set<overloaded_constructor> x;
...
{
    boost_test_hash_set<std::pair<overloaded_constructor, overloaded_constructor> > x;
...
namespace std
{
size_t hash<insert_tests::initialize_from_two_ints>::operator()(const insert_tests::initialize_from_two_ints & val) const
{
    return hash_value(val);
}
size_t hash<insert_tests::overloaded_constructor>::operator()(const insert_tests::overloaded_constructor & val) const
{
    return hash_value(val);
}
}
