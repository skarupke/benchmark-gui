...
static test::seed_t initialize_seed(2974);
    ...
    for (int random_mlf = 0; random_mlf < 2; ++random_mlf)
    {
        for (int i = 1; i < 2000; i += i < 50 ? 1 : 13)
        {
            ...
            x.max_load_factor(random_mlf ?
				static_cast<float>(std::rand() % 1000) / 2000.0f + 0.5f : 1.0f);
...
boost_test_hash_set<int>* int_set_ptr;
...
boost_test_hash_map1<test::movable, test::movable,
	test::hash, test::equal_to,
    test::allocator2<test::movable> >* test_flat_map_ptr;
...
UNORDERED_TEST(rehash_empty_test1,
    ((int_set_ptr)/*(test_multiset_ptr)(test_map_ptr)*/(test_flat_map_ptr)/*(int_multimap_ptr)*/)
)
