...
namespace noexcept_tests
{
    ...
    UNORDERED_AUTO_TEST(test_noexcept)
    {
        ...
        BOOST_TEST((std::is_nothrow_move_constructible<
                    boost_test_hash_set<int> >::value));
		//BOOST_TEST((std::is_nothrow_move_constructible<
		//            boost::unordered_multiset<int> >::value));
		BOOST_TEST((std::is_nothrow_move_constructible<
                    boost_test_hash_map1<int, int> >::value));
		//BOOST_TEST((std::is_nothrow_move_constructible<
		//            boost::unordered_multimap<int, int> >::value));
#endif

        //BOOST_TEST((!std::is_nothrow_move_constructible<
        //            boost_test_hash_set<int, hash_possible_exception>
        //        >::value));
		//BOOST_TEST((!std::is_nothrow_move_constructible<
    ...
    {
        typedef boost_test_hash_set<int,
                hash_nothrow_move, equal_to_nothrow_move> throwing_set;

		if (std::is_nothrow_move_constructible<throwing_set>::value)
        {
