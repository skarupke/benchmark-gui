...
void container_test(X& r, T const&)
{
    ...
    BOOST_STATIC_ASSERT((boost::is_same<const T, const_iterator_value_type>::value));
...
void unordered_map_test(X& r, Key const& k, T const& v)
{
    ...
    BOOST_STATIC_ASSERT((
        boost::is_same<value_type, std::pair<key_type, T> >::value || boost::is_same<value_type, std::pair<key_type const, T> >::value));
...
void unordered_test(X& x, Key& k, Hash& hf, Pred& eq)
{
    ...
    test::check_return_type<iterator>::convertible(a.erase(q1, q2));
