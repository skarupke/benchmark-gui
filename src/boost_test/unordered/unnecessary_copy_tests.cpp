...
namespace unnecessary_copy_tests
{
    ...
    public:
        ...
        count_copies& operator=(const count_copies& p) // Copy assignment
        {
            tag_ = p.tag_;
            ++copies;
...
{
template<>
struct hash<unnecessary_copy_tests::count_copies>
{
	size_t operator()(const unnecessary_copy_tests::count_copies & cc) const
	{
...
namespace unnecessary_copy_tests
{
    ...
    boost_test_hash_set<count_copies>* set;
    boost::unordered_multiset<count_copies>* multiset;
    boost_test_hash_map1<int, count_copies>* map;
    ...
    UNORDERED_AUTO_TEST(unnecessary_copy_emplace_set_test)
    {
        ...
        reset();
        boost_test_hash_set<count_copies> x;
        count_copies a;
        x.insert(a);
        COPY_COUNT(2); MOVE_COUNT(0);
    ...
    !defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
        ...
        // because the container needs to use both arguments to construct an
        // object to pass to the hash function. if it doesn't construct an
        // object, it couldn't have used both arguments from emplace
	...
	UNORDERED_AUTO_TEST(unnecessary_copy_emplace_map_test)
	{
		...
		reset();
        boost_test_hash_map1<count_copies, count_copies> x;
...
#   else
        if(::unnecessary_copy_tests::count_copies::copies > 2 || ::unnecessary_copy_tests::count_copies::copies < 1)
        {
            BOOST_ERROR("Wrong number of copies.");
            std::cerr
                << "Number of copies: "
                << ::unnecessary_copy_tests::count_copies::copies
                << " expecting 1 or 2" << std::endl;
        }
        MOVE_COUNT(0);
#   endif
#endif
