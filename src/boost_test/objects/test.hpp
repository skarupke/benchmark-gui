...
namespace test
{
	...
	inline std::size_t hash_value(test::object const& x) {
        return hash()(x);
    }

	inline std::size_t hash_value(test::movable const& x) {
        return hash()(x);
    }
    ...
    public:
        ...
        allocator1 & operator=(allocator1 const&) = default;
        ~allocator1()
        {
    ...
    template <class T>
    class ptr : public std::iterator<std::random_access_iterator_tag, T>
    {
    ...
    public:
		...
		const_ptr& operator--() { --ptr_; return *this; }
		const_ptr operator--(int) { const_ptr tmp(*this); --ptr_; return tmp; }
        std::ptrdiff_t operator-(const const_ptr & other) const { return ptr_ - other.ptr_; }
