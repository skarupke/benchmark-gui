...
static test::seed_t initialize_seed(747373);
...
struct insert_test_rehash1 : public insert_test_base<T>
{
    ...
    T init() const {
		...
		size_type initial_elements = static_cast<size_type>(ceil(bucket_count * (double) x.max_load_factor()));
...
struct insert_test_rehash3 : public insert_test_base<T>
{
    ...
    T init() const {
        ...
        rehash_bucket_count = static_cast<size_type>(
			ceil(original_bucket_count * (double) x.max_load_factor()));
		if (rehash_bucket_count) -- rehash_bucket_count;

        size_type initial_elements =
			rehash_bucket_count > 5 ? rehash_bucket_count - 5 : rehash_bucket_count;
