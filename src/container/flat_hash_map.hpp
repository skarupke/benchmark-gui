...
struct sherwood_v3_entry
{
    ...
    sherwood_v3_entry(DistanceType distance_from_desired)
    ...
    DistanceType distance_from_desired = -1;
    static constexpr DistanceType special_end_value = 0;
    union { T value; };
};
...
public:
    ...
    sherwood_v3_table(const sherwood_v3_table & other, const ArgumentAlloc & alloc)
        : EntryAlloc(alloc), Hasher(other), Equal(other), hit_max_lookups_limit(other.hit_max_lookups_limit), _max_load_factor(other._max_load_factor)
    {
    ...
    sherwood_v3_table(sherwood_v3_table && other) noexcept
        : EntryAlloc(std::move(other)), Hasher(std::move(other)), Equal(std::move(other)), hit_max_lookups_limit(other.hit_max_lookups_limit), _max_load_factor(other._max_load_factor)
    {
        swap_pointers(other);
    }
    sherwood_v3_table(sherwood_v3_table && other, const ArgumentAlloc & alloc) noexcept
        : EntryAlloc(alloc), Hasher(std::move(other)), Equal(std::move(other)), hit_max_lookups_limit(other.hit_max_lookups_limit), _max_load_factor(other._max_load_factor)
    {
        swap_pointers(other);
    }
...
private:
    ...
    void swap_pointers(sherwood_v3_table & other)
    {
        ...
        swap(hit_max_lookups_limit, other.hit_max_lookups_limit);
    ...
    template<typename Key, typename... Args>
    SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(DistanceType distance_from_desired, EntryPointer current_entry, Key && key, Args &&... args)
    {
        using std::swap;
        if (num_slots_minus_one == 0 || hit_max_lookups(distance_from_desired) || static_cast<double>(num_elements + 1) / static_cast<double>(bucket_count()) > _max_load_factor)
        {
            grow();
    ...
    void deallocate_data(EntryPointer begin, size_t num_slots_minus_one, DistanceType max_lookups)
    {
...
struct prime_number_hash_policy
{
    ...
    {
        return static_cast<uint8_t>(hash) & (0b11111111 >> (8 - num_bits));
    }
...
struct power_of_two_hash_policy
{
    ...
    uint8_t extra_bits_for_hash(size_t hash) const
    {
        return static_cast<uint8_t>(hash >> log2_of_size) & (0b11111111 >> (8 - num_extra_bits));
    }
...
struct power_of_two_hash_policy_other_bits
{
    template<int num_extra_bits>
    size_t index_for_hash(size_t hash, size_t num_slots_minus_one) const
    {
        return (hash >> num_extra_bits) & num_slots_minus_one;
    }
    size_t keep_in_range(size_t index, size_t num_slots_minus_one) const
    {
        return index & num_slots_minus_one;
...
struct switch_prime_number_hash_policy
{
    ...
    size_t keep_in_range(size_t index, size_t num_slots_minus_one) const
    {
        return index > num_slots_minus_one ? this->template index_for_hash<0>(index, num_slots_minus_one) : index;
    }
