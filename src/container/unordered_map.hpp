...
public:
    ...
    std::pair<iterator, bool> emplace(Key && key, Args &&... args)
    {
        ...
        return emplace_new_key(previous, bucket, std::forward<Key>(key), std::forward<Args>(args)...);
    }
        ...
        *new_buckets = EntryPointer(nullptr) + ptrdiff_t(1);
        ...
        for (EntryPointer * it = new_buckets, * end = it + static_cast<ptrdiff_t>(num_buckets + 1); it != end; ++it)
        {
            for (EntryPointer e = *it; e;)
            {
                ...
                size_t index = hash_policy.template index_for_hash<0>(hash_object(e->value), num_slots_minus_one);
...
private:
    ...
    template<typename... Args>
    SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(EntryPointer * previous, EntryPointer * bucket, Args &&... args)
    {
        using std::swap;
        if (load_factor_for(num_elements + 1) > _max_load_factor)
        {
            grow();
    ...
    float load_factor_for(size_t num_elements) const
    {
        if (num_slots_minus_one)
            return num_elements / static_cast<double>(num_slots_minus_one + 1);
