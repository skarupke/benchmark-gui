...
#include <container/libc++__hash_table.hpp>
#include <functional>
#include <stdexcept>
...
public:
    ...
    __unordered_map_hasher()
        noexcept(std::is_nothrow_default_constructible<_Hash>::value)
        : _Hash() {}
...
public:
    ...
    __unordered_map_equal()
        noexcept(std::is_nothrow_default_constructible<_Pred>::value)
        : _Pred() {}

    __unordered_map_equal(const _Pred& __p)
        noexcept(std::is_nothrow_copy_constructible<_Pred>::value)
        : _Pred(__p) {}
...
public:
    ...
    void operator()(pointer __p) noexcept
    {
        if (__second_constructed)
            __alloc_traits::destroy(__na_, std::addressof(__p->__value_.__cc.second));
        if (__first_constructed)
            __alloc_traits::destroy(__na_, std::addressof(__p->__value_.__cc.first));
        if (__p)
...
union __hash_value_type
{
    ...
    typedef std::pair<key_type, mapped_type>              __nc_value_type;
...
class  __hash_map_iterator
{
    ...
    typedef std::pointer_traits<typename _HashIterator::pointer>      __pointer_traits;
...
public:
    ...
    pointer operator->() const {return std::pointer_traits<pointer>::pointer_to(__i_->__cc);}
...
private:
    ...
    typedef typename std::allocator_traits<allocator_type>::template rebind_alloc<__value_type> __allocator_type;
...
public:
    ...
    unordered_map()
        noexcept(std::is_nothrow_default_constructible<__table>::value)
        {
    ...
    unordered_map(unordered_map&& __u)
        noexcept(std::is_nothrow_move_constructible<__table>::value);
    unordered_map(unordered_map&& __u, const allocator_type& __a);
#endif  // _LIBCPP_HAS_NO_RVALUE_REFERENCES
#ifndef _LIBCPP_HAS_NO_GENERALIZED_INITIALIZERS
    unordered_map(std::initializer_list<value_type> __il);
    unordered_map(std::initializer_list<value_type> __il, size_type __n,
    ...
    unordered_map(std::initializer_list<value_type> __il, size_type __n, const allocator_type& __a)
      : unordered_map(__il, __n, hasher(), key_equal(), __a) {}

    unordered_map(std::initializer_list<value_type> __il, size_type __n, const hasher& __hf,
      const allocator_type& __a)
    ...
    unordered_map& operator=(unordered_map&& __u)
        noexcept(std::is_nothrow_move_assignable<__table>::value);
#endif
#ifndef _LIBCPP_HAS_NO_GENERALIZED_INITIALIZERS
    unordered_map& operator=(std::initializer_list<value_type> __il);
#endif  // _LIBCPP_HAS_NO_GENERALIZED_INITIALIZERS
...
#ifndef _LIBCPP_HAS_NO_RVALUE_REFERENCES
    template <class _Pp,
              class = typename std::enable_if<std::is_constructible<value_type, _Pp>::value>::type>

        std::pair<iterator, bool> insert(_Pp&& __x)
        ...
        std::pair<iterator, bool> try_emplace(const key_type& __k, _Args&&... __args)
    {
        ...
        if ( __p != end())
            ...
            return _VSTD::make_pair(
                      emplace_hint(__p,
                        _VSTD::piecewise_construct, std::forward_as_tuple(__k),
        ...
        std::pair<iterator, bool> try_emplace(key_type&& __k, _Args&&... __args)
    {
        ...
        if ( __p != end())
            ...
            return _VSTD::make_pair(
                      emplace_hint(__p,
                        _VSTD::piecewise_construct, std::forward_as_tuple(std::move(__k)),
                        std::forward_as_tuple(std::forward<_Args>(__args)...)),
                      true);
    }
        ...
        std::pair<iterator, bool> insert_or_assign(const key_type& __k, _Vp&& __v)
    {
        ...
        if ( __p != end())
        {
            __p->second = std::move(__v);
            return _VSTD::make_pair(__p, false);
        }
        return _VSTD::make_pair(emplace_hint(__p, __k, std::forward<_Vp>(__v)), true);
    }
        ...
        std::pair<iterator, bool> insert_or_assign(key_type&& __k, _Vp&& __v)
    {
        ...
        return _VSTD::make_pair(emplace_hint(__p, std::forward<key_type>(__k), std::forward<_Vp>(__v)), true);
    }
    ...
    template <class _Vp>
        ...
        iterator insert_or_assign(const_iterator __h, const key_type& __k, _Vp&& __v)
     {
        ...
        return emplace_hint(__h, __k, std::forward<_Vp>(__v));
     }
    ...
    template <class _Vp>
        ...
        iterator insert_or_assign(const_iterator __h, key_type&& __k, _Vp&& __v)
     {
        ...
        return emplace_hint(__h, std::forward<key_type>(__k), std::forward<_Vp>(__v));
     }
#endif
#endif
#endif
    ...
    inline iterator       find(const key_type& __k)       {return __table_.find(__k);}

    inline const_iterator find(const key_type& __k) const {return __table_.find(__k);}
    ...
    if (__a != __u.get_allocator())
    {
        ...
        while (__u.size() != 0)
            __table_.__insert_unique(
                std::move(__u.__table_.remove((__i++).__i_)->__value_)
                                    );
    }
#if _LIBCPP_DEBUG_LEVEL >= 2
    else
...
template <class _Key, class _Tp, class _Hash, class _Pred, class _Alloc>
unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::unordered_map(
        std::initializer_list<value_type> __il, size_type __n, const hasher& __hf,
        const key_equal& __eql)
    : __table_(__hf, __eql)
{
#if _LIBCPP_DEBUG_LEVEL >= 2
...
unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::__construct_node()
{
    ...
    __node_traits::construct(__na, std::addressof(__h->__value_));
...
unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::__construct_node_with_key(key_type&& __k)
{
    __node_allocator& __na = __table_.__node_alloc();
    __node_holder __h(__node_traits::allocate(__na, 1), _Dp(__na));
    __node_traits::construct(__na, std::addressof(__h->__value_.__cc.first), std::move(__k));
    __h.get_deleter().__first_constructed = true;
    __node_traits::construct(__na, std::addressof(__h->__value_.__cc.second));
    __h.get_deleter().__second_constructed = true;
    return __h;
}
                                                                 ...
                                                                 _Args&&... __args)
{
    ...
    __node_holder __h(__node_traits::allocate(__na, 1), _Dp(__na));
    __node_traits::construct(__na, std::addressof(__h->__value_),
                             std::forward<_A0>(__a0), std::forward<_A1>(__a1),
...
template <class _Key, class _Tp, class _Hash, class _Pred, class _Alloc>
template <class... _Args>
std::pair<typename unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::iterator, bool>
...
unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::__construct_node_with_key(const key_type& __k)
{
    __node_allocator& __na = __table_.__node_alloc();
    __node_holder __h(__node_traits::allocate(__na, 1), _Dp(__na));
    __node_traits::construct(__na, std::addressof(__h->__value_.__cc.first), __k);
    __h.get_deleter().__first_constructed = true;
    __node_traits::construct(__na, std::addressof(__h->__value_.__cc.second));
    __h.get_deleter().__second_constructed = true;
...
unordered_map<_Key, _Tp, _Hash, _Pred, _Alloc>::operator[](key_type&& __k)
{
    ...
    if (__i != end())
        return __i->second;
    __node_holder __h = __construct_node_with_key(std::move(__k));
    std::pair<iterator, bool> __r = __table_.__node_insert_unique(__h.get());
    __h.release();
    return __r.first->second;
}
