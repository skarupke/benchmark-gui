...
struct sherwood_v11_block
{
    ...
    static sherwood_v11_block * empty_block()
    {
        static sherwood_v11_block * empty_block = []
        {
            static sherwood_v11_block blocks[2];
            blocks[0].control_bytes = blocks[1].control_bytes = _mm_set_epi8(
                        sherwood_v11_constants<>::magic_for_end_aligned,
                        sherwood_v11_constants<>::magic_for_empty,
                        sherwood_v11_constants<>::magic_for_empty,
                        sherwood_v11_constants<>::magic_for_empty,
                        sherwood_v11_constants<>::magic_for_empty,
                        sherwood_v11_constants<>::magic_for_empty,
    ...
    {
        int empty_indices = _mm_movemask_epi8(_mm_and_si128(control_bytes, _mm_set1_epi8(sherwood_v11_constants<>::magic_for_empty)));
        if (empty_indices)
    ...
    {
        return _mm_movemask_epi8(_mm_cmpeq_epi8(control_bytes, _mm_set1_epi8(sherwood_v11_constants<>::magic_for_empty))) != 0;
    }
};

template<typename T, typename FindKey, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename Equal, typename ArgumentAlloc, typename ByteAlloc>
class sherwood_v11_table : private ByteAlloc, private Hasher, private Equal
{
    ...
    using BlockPointer = sherwood_v11_block<T> *;
...
public:
        ...
        : ByteAlloc(alloc), Hasher(other), Equal(other), _max_load_factor(other._max_load_factor)
    {
        ...
        {
            clear();
            deallocate_data(entries, num_blocks_minus_one);
            throw;
        }
    }
    ...
    struct templated_iterator
    {
        ...
        templated_iterator & operator++()
        {
            ...
            while(sherwood_v11_constants<>::skip_while_iterating(current->control_bytes_as_ints[position_in_block]));
            return *this;
        }
    ...
    iterator begin()
    {
        if (entries == sherwood_v11_block<T>::empty_block())
            return {entries + 1, 0};
        else if (sherwood_v11_constants<>::skip_while_iterating(entries[0].control_bytes_as_ints[0]))
    ...
    iterator end()
    {
        return { entries + num_blocks_minus_one + 1, 0 };
    }
    ...
    inline iterator find(const FindKey & key)
    {
        ...
        static constexpr int num_extra_bits = sherwood_v11_constants<>::num_extra_bits;
    ...
    void rehash(size_t num_items)
    {
        ...
        if (entries != sherwood_v11_block<T>::empty_block() && num_blocks == num_blocks_minus_one + 1)
            return;
        ...
        for (BlockPointer it = new_buckets; it != special_end_item; ++it)
            it->control_bytes = _mm_set1_epi8(sherwood_v11_constants<>::magic_for_empty);
        if (is_misaligned)
            special_end_item->control_bytes = _mm_set1_epi8(sherwood_v11_constants<>::magic_for_end_misaligned);
        else
            special_end_item->control_bytes = _mm_set1_epi8(sherwood_v11_constants<>::magic_for_end_aligned);
        using std::swap;
        ...
        num_elements = 0;
        num_tombstones = 0;
        if (new_buckets != sherwood_v11_block<T>::empty_block())
        {
            for (BlockPointer it = new_buckets, end = new_buckets + num_blocks + 1; it != end; ++it)
            {
                for (int i = 0; i < 16; ++i)
                {
                    if (!sherwood_v11_constants<>::skip_while_iterating(it->control_bytes_as_ints[i]))
                    {
            ...
            deallocate_data(new_buckets, num_blocks);
        }
    }
    ...
    convertible_to_iterator erase(const_iterator to_erase)
    {
        ...
        --num_elements;
        uint8_t new_metadata = sherwood_v11_constants<>::magic_for_empty;
        if (!current->any_empty())
        {
            new_metadata = sherwood_v11_constants<>::magic_for_tombstone;
            ++num_tombstones;
        }
        current->control_bytes_as_ints[to_erase.position_in_block] = new_metadata;
    ...
    void clear()
    {
        if (entries == sherwood_v11_block<T>::empty_block())
            return;
        for (BlockPointer it = entries, end = it + num_blocks_minus_one + 1; it != end; ++it)
        {
            for (int i = 0; i < 16; ++i)
            {
                if (it->control_bytes_as_ints[i] == sherwood_v11_constants<>::magic_for_tombstone)
                    it->control_bytes_as_ints[i] = sherwood_v11_constants<>::magic_for_empty;
                else if (it->control_bytes_as_ints[i] != sherwood_v11_constants<>::magic_for_empty)
                {
    ...
    {
        return hash_policy.template index_for_hash<sherwood_v11_constants<>::num_extra_bits>(hash_object(key), num_blocks_minus_one);
    }
...
private:
    BlockPointer entries = sherwood_v11_block<T>::empty_block();
    ...
    SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(size_t index, uint8_t extra_bits, Args &&... args)
    {
        ...
        {
            BlockPointer block = entries + index;
            int first_empty_index = block->first_empty_index();
            if (first_empty_index != -1)
            {
                AllocatorTraits::construct(*this, std::addressof(block->data[first_empty_index]), std::forward<Args>(args)...);
                if (block->control_bytes_as_ints[first_empty_index] == sherwood_v11_constants<>::magic_for_tombstone)
                    --num_tombstones;
                block->control_bytes_as_ints[first_empty_index] = extra_bits;
                ++num_elements;
                return { { block, first_empty_index }, true };
            }
        }
    }
    ...
    {
        size_t memory_required = sizeof(sherwood_v11_block<T>) * num_blocks;
    ...
    {
        if (begin == sherwood_v11_block<T>::empty_block())
            return;

        size_t memory = calculate_memory_requirement(num_blocks_minus_one + 1);
        BlockPointer end = begin + num_blocks_minus_one + 1;
        unsigned char * as_byte_pointer = reinterpret_cast<unsigned char *>(begin);
        if (end->control_bytes_as_ints[0] == sherwood_v11_constants<>::magic_for_end_misaligned)
            as_byte_pointer -= 8;
    ...
    {
        deallocate_data(entries, num_blocks_minus_one);
        entries = sherwood_v11_block<T>::empty_block();
    ...
    struct convertible_to_iterator
    {
        ...
        operator iterator()
        {
            if (sherwood_v11_constants<>::skip_while_iterating(it->control_bytes_as_ints[position_in_block]))
        ...
        {
            if (sherwood_v11_constants<>::skip_while_iterating(it->control_bytes_as_ints[position_in_block]))
                return ++const_iterator{it, position_in_block};
            else
                return { it, position_in_block };
        }
    };
...
public:
    ...
    friend bool operator==(const google_block_hash_map & lhs, const google_block_hash_map & rhs)
    {
...
public:
    ...
    friend bool operator==(const google_block_hash_set & lhs, const google_block_hash_set & rhs)
    {
        if (lhs.size() != rhs.size())
            return false;
