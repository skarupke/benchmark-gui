...
struct sherwood_v8_block
{
    ...
    static sherwood_v8_block * empty_block()
    {
        static std::array<int8_t, BlockSize> empty_bytes = []
        {
            ...
            std::fill(result.begin(), result.end(), sherwood_v8_constants::magic_for_empty);
            return result;
        }();
...
template<typename T, typename FindKey, typename ArgumentHash, typename Hasher, typename ArgumentEqual, typename Equal, typename ArgumentAlloc, typename ByteAlloc, LookupType lookup_type, uint8_t BlockSize>
class sherwood_v8_table : private ByteAlloc, private Hasher, private Equal
{
    ...
    using BlockPointer = sherwood_v8_block<T, BlockSize> *;
...
public:
    ...
    inline iterator find(const FindKey & key)
    {
        ...
        if constexpr (lookup_type == MoreOutside)
        {
            index = hash_policy.template index_for_hash<0>(index, num_slots_minus_one);
            ...
            if (compares_equal(key, block->data[index_in_block]))
                return { block, index };
            for (int8_t to_next_index = metadata; to_next_index; to_next_index = block->control_bytes[index_in_block] & Constants::bits_for_distance)
            {
                index += Constants::jump_distances[to_next_index];
                index = hash_policy.keep_in_range(index, num_slots_minus_one);
                block_index = index / BlockSize;
                index_in_block = index % BlockSize;
                block = entries + block_index;
                if (compares_equal(key, block->data[index_in_block]))
                    return { block, index };
            }
            return end();
        }
        else if constexpr (lookup_type == SomeOutside)
        {
            index = hash_policy.template index_for_hash<0>(index, num_slots_minus_one);
            size_t block_index = index / BlockSize;
            int index_in_block = index % BlockSize;
            ...
            for (;;)
            {
                ...
                index_in_block = index % BlockSize;
                block = entries + block_index;
                metadata = block->control_bytes[index_in_block];
            }
        }
        else
        {
    ...
    // debug function: returns how often each jump distance is used
    ska::flat_hash_map<int, int> jump_distance_counts() const
    {
        ska::flat_hash_map<int, int> result;
        ...
        for (BlockPointer it = entries, end = it + num_blocks; it != end; ++it)
        {
            for (int i = 0; i < BlockSize; ++i)
            {
                ...
                {
                    ++result[it->control_bytes[i] & Constants::bits_for_distance];
                }
            }
        }
        return result;
    ...
    inline std::pair<iterator, bool> emplace(Key && key, Args &&... args)
    {
        ...
        size_t num_slots_minus_one = this->num_slots_minus_one;
        BlockPointer entries = this->entries;
        if constexpr (lookup_type == SomeOutside)
        {
            index = hash_policy.template index_for_hash<0>(index, num_slots_minus_one);
            size_t block_index = index / BlockSize;
            int index_in_block = index % BlockSize;
...
private:
    BlockPointer entries = sherwood_v8_block<T, BlockSize>::empty_block();
    ...
    LinkedListIt find_direct_hit(LinkedListIt child) const
    {
        ...
        size_t to_move_index = hash_policy.template index_for_hash<0>(to_move_hash, num_slots_minus_one);
    ...
    {
        size_t memory_required = sizeof(sherwood_v8_block<T, BlockSize>) * num_blocks;
    ...
    {
        if (begin == sherwood_v8_block<T, BlockSize>::empty_block())
            return;
    ...
    {
        deallocate_data(entries, num_slots_minus_one);
        entries = sherwood_v8_block<T, BlockSize>::empty_block();
...
template<typename K, typename V, typename H = std::hash<K>, typename E = std::equal_to<K>, typename A = std::allocator<std::pair<K, V> >, detailv8::LookupType lookup_type = detailv8::Default, size_t BlockSize = detailv8::CalculateBytellBlockSize<K, V>::value >
...
template<typename T, typename H = std::hash<T>, typename E = std::equal_to<T>, typename A = std::allocator<T>, detailv8::LookupType lookup_type = detailv8::Default, size_t BlockSize = detailv8::CalculateBytellBlockSize<T>::value >
