...
#define CRASH_DIVIDER(INT, TYPE) \
    ...
    LIBDIVIDE_API uint32_t libdivide_u32_crash(uint32_t, const libdivide_u32_t *) { exit(-1); }
    LIBDIVIDE_API uint64_t libdivide_u64_crash(uint64_t, const libdivide_u64_t *) { exit(-1); }

#if defined(LIBDIVIDE_USE_SSE2)
    LIBDIVIDE_API __m128i libdivide_u32_crash_vector(__m128i, const libdivide_u32_t *) { exit(-1); }
    LIBDIVIDE_API __m128i libdivide_u64_crash_vector(__m128i, const libdivide_u64_t *) { exit(-1); }
#endif
